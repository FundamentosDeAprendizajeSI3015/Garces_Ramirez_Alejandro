...
from sklearn.model_selection import (
    train_test_split, cross_val_score, StratifiedKFold, GridSearchCV, learning_curve
)
...
# ==========================================
# CICLO DE VIDA DE MACHINE LEARNING (ML) - APUNTES
# 1) Definición del problema:
#    Clasificar flores Iris en 3 especies usando medidas (features).
#
# 2) Recolección / carga de datos:
#    Se obtiene el dataset Iris desde scikit-learn.
#
# 3) Exploración de datos (EDA):
#    Dimensiones, estadísticas, distribución de clases, correlación y gráficos.
#
# 4) Ingeniería de características:
#    Crear nuevas variables (ratios/áreas) a partir de las originales.
#
# 5) Preprocesamiento:
#    Escalado (StandardScaler) dentro de pipelines para modelos que lo requieren.
#
# 6) Partición de datos:
#    Separación en train/test y validación cruzada estratificada.
#
# 7) Entrenamiento:
#    Entrenar varios modelos y comparar desempeño.
#
# 8) Evaluación:
#    Métricas en test, matriz de confusión y (si aplica) ROC-AUC.
#
# 9) Ajuste de hiperparámetros:
#    GridSearchCV para mejorar SVM y RandomForest.
#
# 10) Selección del mejor modelo:
#     Elegir el que tenga mejor performance en CV.
#
# 11) Persistencia / despliegue:
#     Guardar el modelo en un .joblib para reutilizarlo.
# ==========================================
...
# (AGREGADO) Guardar el ciclo de vida en un archivo de texto para apuntes
with open("outputs/ml_lifecycle.txt", "w", encoding="utf-8") as f:
    f.write("""CICLO DE VIDA DE MACHINE LEARNING (ML)

1) Definición del problema:
   - Clasificar flores Iris (setosa, versicolor, virginica) usando medidas.

2) Recolección / carga de datos:
   - Dataset Iris cargado desde scikit-learn.

3) Exploración de datos (EDA):
   - Revisar dimensiones, estadísticas, clases, correlación y visualizaciones.

4) Ingeniería de características:
   - Crear features nuevas a partir de las originales (ratios y áreas).

5) Preprocesamiento:
   - Escalar con StandardScaler (en pipelines).

6) Partición:
   - Split train/test (estratificado) + validación cruzada estratificada.

7) Entrenamiento:
   - Probar varios modelos (LR, KNN, DT, RF, SVM).

8) Evaluación:
   - Accuracy, precision, recall, F1, matriz de confusión, ROC-AUC (si aplica).
...
11) Persistencia:
   - Guardar el modelo con joblib para reutilizarlo.
""")
...
# ==========================================
# (AGREGADO) ANÁLISIS EXTRA SENCILLO: CURVA DE APRENDIZAJE
# Muestra si el modelo mejora con más datos y si hay overfitting/underfitting.
# ==========================================
try:
    ...
    print("No se pudo generar la curva de aprendizaje:", e)
...
with open("outputs/summary.txt", "w", encoding="utf-8") as f:
    ...
    f.write("\nSe generó también:\n")
    f.write("- outputs/ml_lifecycle.txt (ciclo de vida de ML)\n")
    f.write("- outputs/learning_curve.png (análisis extra: curva de aprendizaje)\n")

print("\nGráficos y artefactos guardados en la carpeta 'outputs'.")
print("Ciclo de vida guardado en outputs/ml_lifecycle.txt")
print("Curva de aprendizaje guardada en outputs/learning_curve.png")